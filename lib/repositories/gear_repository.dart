...
class GearRepository {
  ...
  Future<List<Gear>> getAllGear() async {
    final db = await _dbHelper.database;
    final maps = await db.query('gear', orderBy: 'name ASC');
  ...
  Future<Gear?> getGearById(int id) async {
    final db = await _dbHelper.database;
    final maps = await db.query('gear', where: 'id = ?', whereArgs: [id]);
  ...
  Future<int> updateGear(Gear gear) async {
    final db = await _dbHelper.database;
    return await db.update('gear', gear.toMap(), where: 'id = ?', whereArgs: [gear.id]);
  }
  ...
  Future<void> setGearForEntry(int entryId, List<int> gearIds) async {
    ...
    await db.transaction((txn) async {
      await txn.delete('entry_gear', where: 'entry_id = ?', whereArgs: [entryId]);
      for (final gearId in gearIds) {
        await txn.insert('entry_gear', {'entry_id': entryId, 'gear_id': gearId});
      }
    });
  }
  ...
  // GET all entry IDs linked to a gear item
  Future<Set<int>> getEntryIdsForGear(int gearId) async {
    final db = await _dbHelper.database;
    ...
    return result.map((r) => r['entry_id'] as int).toSet();
  }
  ...
  Future<GearStats> getGearStats(int gearId) async {
    ...
    if (gear == null) throw Exception('Gear not found');
